...
library(kableExtra)
library(cowplot)
# library(parallel) # uncomment if using detectCores()
```
...
# num.cores.available=detectCores()  # Multithreading on quest analytics node or personal laptop. Load parallel library first.
...
resolution.selection <- "RNA_snn_res.0.25"
```
...
load(file=file.path(r.obj.loc,"clustered_seurat_object_and_markers.RData"))

ggplot.directory <- file.path(r.obj.loc, "ggplot")
dir.create(ggplot.directory, showWarnings = F)

resolution.selection <- "RNA_snn_res.0.25"
```

```{r load_rdata, warning=FALSE,error=FALSE,message=FALSE, echo = !run_tests, eval = !run_tests}
load(file=file.path(r.obj.loc,"clusters_seurat_object.RData"))
...
cluster.umap <- DimPlot(object = experiment.aggregate, pt.size=0.5, reduction = "umap", label = T)
...
```{r find_all_markers, warning=FALSE,error=FALSE,message=FALSE, eval = !run_tests}
...
feat_plots_top_genes <- function(cluster.num){  
  ...
  # feat.plot <- cowplot::plot_grid(plotlist = feat.plot.list)
  plot.title <- ggdraw() + draw_label(paste0("Cluster ", cluster.num), fontface = 'bold')
  print(cowplot::plot_grid(plot.title, feat.plot.list, ncol = 1, rel_heights = c(0.1, 1)))
   
  ggsave(feat.plot.path)
...
save(list=save.for.rdata, file=paste0(r.obj.loc,"clustered_seurat_object_and_markers.RData"))

to.save.for.report <- c("cellcycle.umap",
                        "cluster.umap",
                        "markers_all",
                        "nGenes.featureplot",
                        "nUMI.featureplot",
...
save(list = to.save.for.report, file=paste0(r.obj.loc, "part5_stuff_for_report.RData"))
