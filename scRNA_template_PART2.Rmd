...
```{r user_vals, warning=FALSE,error=FALSE,message=FALSE}
r.obj.loc <- "r_object_path"
organism="mouse" # Used in calculating cell cycle below

mito.cutoff = 0 
RNA.count.floor = 0
RNA.count.ceiling = 0
feature.count.cutoff = 0
```
...
<!-- ```{r test_load, warning=FALSE,error=FALSE,message=FALSE} -->
<!-- r.obj.loc <- "/Volumes/My_Passport_for_Mac/tenX_projects/Bonini07/rds/" -->
<!-- load(file=paste0(r.obj.loc, "original_seurat_object.RData")) -->
<!-- organism="mouse" -->
<!-- set.seed(12345) -->

<!-- mito.cutoff = 25  -->
<!-- RNA.count.floor = 1000 -->
<!-- RNA.count.ceiling = 60000 -->
<!-- feature.count.cutoff = 700 -->
<!-- ``` -->
...
ridgeplot.prefiltering <- RidgePlot(experiment.aggregate, features=c("nFeature_RNA","nCount_RNA", "percent.mito"), ncol = 2)
...
FeatureScatter(experiment.aggregate, feature1 = "nCount_RNA", feature2 = "percent.mito") + geom_vline(xintercept = c(RNA.count.floor,RNA.count.ceiling)) + geom_hline(yintercept = mito.cutoff)

FeatureScatter(experiment.aggregate, feature1 = "nFeature_RNA", feature2 = "percent.mito") + geom_vline(xintercept = feature.count.cutoff) + geom_hline(yintercept = mito.cutoff)

FeatureScatter(
  experiment.aggregate, "nCount_RNA", "nFeature_RNA",
  pt.size = 0.5)  + geom_vline(xintercept = c(RNA.count.floor,RNA.count.ceiling)) + geom_hline(yintercept = feature.count.cutoff)
...
experiment.aggregate <- subset(experiment.aggregate, nFeature_RNA >= feature.count.cutoff)
...
```{r our_variable_genes, warning=FALSE,error=FALSE,message=FALSE, eval=FALSE}
