...
#   sweep.res.list <- paramSweep_v3(single.seurat.object, 
#                                   PCs = 1:num.pcs, 
#                                   sct = run.sctransform) 
#   
...
# #   sweep.res.list <- paramSweep_v3(single.seurat.object, PCs = 1:10, sct = TRUE) 
# #   gt.calls <- single.seurat.object@meta.data[rownames(sweep.res.list[[1]]), "GT"].   ## GT is a vector containing "Singlet" and "Doublet" calls recorded using sample multiplexing classification and/or in silico geneotyping results 
# # sweep.stats <- summarizeSweep(sweep.res.list, GT = TRUE, GT.calls = gt.calls)
# # bcmvn <- find.pK(sweep.stats)
# 
# ## Homotypic Doublet Proportion Estimate ------------------------------------------------------------------------------------- 
#   annotations <- single.seurat.object@meta.data$integrated_snn_res.0.2
#   homotypic.prop <- modelHomotypic(annotations)           ## ex: annotations <- single.seurat.object@meta.data$ClusteringResults 
...
# 
# ## Run DoubletFinder with varying classification stringenciesG ----------------------------------------------------------------
#   single.seurat.object <- doubletFinder_v3(single.seurat.object, 
#                                            PCs = 1:10, 
#                                            pN = 0.25, 
#                                            pK = 0.09, 
#                                            nExp = nExp_poi, 
...
#   
#   
# # # single.seurat.object <- doubletFinder_v3(single.seurat.object, PCs = 1:10, pN = 0.25, pK = 0.09, nExp = nExp_poi.adj, reuse.pANN = "pANN_0.25_0.09_913", sct = TRUE)
# # 
# #   curr.sample.name <- unique(single.seurat.object$orig.ident)
# #   print(paste0("Sample ", curr.sample.name))
# #   
# #   
# #   nExp <- round(ncol(single.seurat.object) * 0.04)  # expect 4% doublets
# #   sweep.list <- paramSweep_v3(single.seurat.object, PCs = 1:num.pcs, sct=TRUE)
...
sweep.res.list <- paramSweep_v3(experiment.aggregate, 
                                  PCs = 1:num.pcs, 
                                  sct = run.sctransform) 
...
bcmvn <- find.pK(sweep.stats) 
  ...
  ## pK Identification (ground-truth) ------------------------------------------------------------------------------------------ 
#   sweep.res.list <- paramSweep_v3(experiment.aggregate, PCs = 1:10, sct = TRUE) 
#   gt.calls <- experiment.aggregate@meta.data[rownames(sweep.res.list[[1]]), "GT"].   ## GT is a vector containing "Singlet" and "Doublet" calls recorded using sample multiplexing classification and/or in silico geneotyping results 
# sweep.stats <- summarizeSweep(sweep.res.list, GT = TRUE, GT.calls = gt.calls)
# bcmvn <- find.pK(sweep.stats)

## Homotypic Doublet Proportion Estimate ------------------------------------------------------------------------------------- 
annotations <- experiment.aggregate@meta.data$integrated_snn_res.0.2
homotypic.prop <- modelHomotypic(annotations)           ## ex: annotations <- experiment.aggregate@meta.data$ClusteringResults 
nExp_poi <- round(0.075*nrow(experiment.aggregate@meta.data))  ## Assuming 7.5% doublet formation rate - tailor for your dataset 
nExp_poi.adj <- round(nExp_poi*(1-homotypic.prop))

## Run DoubletFinder with varying classification stringenciesG ----------------------------------------------------------------
experiment.aggregate <- doubletFinder_v3(experiment.aggregate,  
                                         PCs = 1:10,  
                                         pN = 0.25,  
                                         pK = 0.09,  
                                         nExp = nExp_poi,  
