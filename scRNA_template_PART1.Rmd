...
config.params <- read_yaml("config.yaml")
analyst.name <- config.params$project$`analyst-name`
formatted.samples <- config.params$project$samples

formatted.samples <- gsub("\"", "", formatted.samples)
formatted.samples <- gsub("\ ", " </br> ", formatted.samples)
formatted.samples <- gsub("-", " * ", formatted.samples)

project.description <- config.params$project$`project-decription`
```
...
```{r}
run_tests <- config.params$analysis$run_tests
import_h5 <- config.params$data$`import-h5` # set to false if you want to import the matrix files
...
experiment_name = config.params$project$`project-name` 
...
dataset_loc <- config.params$data$`h5-file-path` # path containing the folders containing the raw_feature_bc_matrix.h5 files

organism = config.params$project$organism
...
r.obj.loc <- config.params$data$`rds-file-path` # Output folder

ids <- config.params$project$samples # sample names, need to match the folders containing the h5 files
ids <- gsub("-", "", ids)
ids <- gsub("\"", "", ids)
...
dataset_loc <- "/Users/brianwray/Desktop/Projects/LePoole01/Matrix"
...
d10x.metrics <- lapply(ids, function(i){
  print(i)
  metrics.path <- file.path(dataset_loc,i,"metrics_summary.csv")
  metrics <- read.csv(metrics.path, colClasses = "character")
})
...
```{r load_data_hdf5, warning=FALSE,error=FALSE, message=FALSE, eval = import_h5, echo = import_h5}
...
```{r load_data_matrix, warning=FALSE,error=FALSE, message=FALSE, eval = !import_h5, echo = !import_h5}
...
relative.expression.boxplot <- boxplot(as.matrix(t(C[most_expressed, ])), cex = 0.1, las = 1, xlab = "% total count per cell",
...
save(list=c("sequencing.char.table",  
            ...
            "part1.vln.plots",
            "relative.expression.boxplot"), 
     file=paste0(r.obj.loc, "part1_stuff_for_report.RData"))
```
