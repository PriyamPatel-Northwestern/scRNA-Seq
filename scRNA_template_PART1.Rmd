...
### Setup the experiment folder and data info, and grab args from config file
...
config.args <- read_yaml("config.yaml")
...
part1.rds.save.file <- file.path(r.obj.loc, config.args$analysis$part1$part1_rds_save_filename)

part1.file.for.report <-file.path(r.obj.loc, config.args$analysis$part1$part1_report_stuff_filename) 
...
import_h5 <- config.args$data$`import-h5` # set to false if you want to import the matrix files
...
d10x.metrics <- lapply(ids, function(i){
  print(i)
  metrics.path <- file.path(dataset.path,i,"metrics_summary.csv")
...
d10x.data <- lapply(ids, function(i){
  ...
  if (import_h5){
    raw.data.path <- file.path(dataset.path,i,"raw_feature_bc_matrix.h5")
    d10x <- Read10X_h5(raw.data.path)
  } else {
    raw.data.path <- file.path(dataset.path,i,"raw_feature_bc_matrix")
    d10x <- Read10X(raw.data.path)
  }
...
saveRDS(experiment.aggregate,file=part1.rds.save.file)
